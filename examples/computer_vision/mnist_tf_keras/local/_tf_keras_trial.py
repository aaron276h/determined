...
class LoadWeightsAndOptimizer(tf.keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if not self.loaded:
            self.loaded = True
            logging.info(f"Restoring checkpoint from {self.load_path}")
            self.model.load_weights(str(self.load_path))
            load_optimizer_weights(self.model, self.load_path)
...
class TFKerasTrialController(det.LoopTrialController):
    ...
    def _load(self) -> None:
        ...
        if self.load_path.joinpath("determined-keras-weights.h5").exists():
            full_ckpt_path = self.load_path.joinpath("determined-keras-weights.h5")
            optimizer_ckpt_path = self.load_path.joinpath("determined-keras-optimizer.h5")
        elif self.load_path.joinpath("determined-keras-model.h5").exists():
        ...
        with h5py.File(optimizer_ckpt_path, "r") as h5file:
            for idx, optimizer in enumerate(self.context.optimizers):
                optimizer._create_all_weights(self.model.trainable_variables)

                group_name = f"optimizers-{idx}"
                optimizer_values = load_optimizer_weights_from_hdf5_group(h5file[group_name])
        ...
        #self.tf_keras_callbacks.append(LoadWeightsAndOptimizer(full_ckpt_path))
        #self.model.load_weights(str(full_ckpt_path))
    ...
    def _save_checkpoint(self, path: pathlib.Path) -> workload.Response:
        ...
        # Save model.
        #self.model.save(path.joinpath("determined-keras-model"), save_format="saved_model")
        self.model.save_weights(str(path.joinpath("determined-keras-weights.h5")), save_format="h5")

        with h5py.File(path.joinpath("determined-keras-optimizer.h5"), "w") as h5file:
            for idx, optimizer in enumerate(self.context.optimizers):
                opt_group = h5file.create_group(f"optimizers-{idx}")
